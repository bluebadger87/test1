...
namespace _26_Thread
{
    public partial class Form1 : Form
    {
        ...
        {
            _locationX = this.Location.X + this.Size.Width; //진행바 나타낼 x위치
            _locationY = this.Location.Y; //진행바 나타낼 y위치

            for (int i = 0; i < numPlayerCount.Value; i++)
            {
                Play pl = new Play(((enumPlayer)i).ToString()); //enum값이 들어감
                pl.Location = new Point(_locationX, _locationY + pl.Height * i); //좌표값
